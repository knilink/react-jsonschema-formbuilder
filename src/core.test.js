const { schema2tree, removeNodeByPath, addNodeByPath, moveNodeByPath, updateNodeByPath } = require('./core');
describe('core functions', function() {
  ...
  describe('Remove tree node', function() {
    function r(before, after, key) {
      ...
      const expectedTreeAfter = schema2tree('root', after.schema, after.uiSchema);
  ...
  describe('add tree node', function() {
    function r(before, after, key, position, add, name) {
      ...
      const treeAfter = addNodeByPath(treeBefore, key.split('.'), position, nodeAdd);
      const expectedTreeAfter = schema2tree('root', after.schema, after.uiSchema);
      expect(treeAfter).toEqual(expectedTreeAfter);
    }
  ...
  describe('Move tree node', function() {
    function r(before, after, source, target, position) {
      const treeBefore = schema2tree('root', before.schema, before.uiSchema);
      const treeAfter = moveNodeByPath(treeBefore, source.split('.'), target.split('.'), position);
      const expectedTreeAfter = schema2tree('root', after.schema, after.uiSchema);
      expect(treeAfter).toEqual(expectedTreeAfter);
    }
    ...
    it('move array items within array', () => {
      const before = {
        schema: {
          type: 'array',
          items: [{ name: 'foo', type: 'string' }, { name: 'bar', type: 'number' }, { name: 'baz', type: 'boolean' }],
        },
        uiSchema: {
          items: [{ 'ui:foo': 'foo' }, { 'ui:bar': 'bar' }, { 'ui:baz': 'baz' }],
        },
      };
      const moveForwardAfter = {
        schema: {
          type: 'array',
          items: [{ name: 'foo', type: 'string' }, { name: 'baz', type: 'boolean' }, { name: 'bar', type: 'number' }],
        },
        uiSchema: {
          items: [{ 'ui:foo': 'foo' }, { 'ui:baz': 'baz' }, { 'ui:bar': 'bar' }],
        },
      };
      const moveBackwardAfter = {
        schema: {
          type: 'array',
          items: [{ name: 'bar', type: 'number' }, { name: 'foo', type: 'string' }, { name: 'baz', type: 'boolean' }],
        },
        uiSchema: {
          items: [{ 'ui:bar': 'bar' }, { 'ui:foo': 'foo' }, { 'ui:baz': 'baz' }],
        },
      };
    ...
    it('should not drop under leaf', function() {
      ...
      const tree = schema2tree('root', data.schema, data.uiSchema);
      expect(moveNodeByPath(tree, 'root.baz'.split('.'), 'root.foo'.split('.'), 0)).toBe(tree);
      expect(moveNodeByPath(tree, 'root.bar.barr'.split('.'), 'root.foo'.split('.'), 0)).toBe(tree);
      expect(moveNodeByPath(tree, 'root.foo'.split('.'), 'root.bar.barr'.split('.'), 0)).toBe(tree);
    });
  ...
  describe('update tree node', function() {
    function r(before, after, key, nodeUpdate) {
      const treeBefore = schema2tree('root', before.schema, before.uiSchema);
      const treeAfter = updateNodeByPath(treeBefore, key.split('.'), nodeUpdate);
      const expectedTreeAfter = schema2tree('root', after.schema, after.uiSchema);
      expect(treeAfter).toEqual(expectedTreeAfter);
    }
    ...
    it('update root name', function() {
      ...
      const treeAfter = updateNodeByPath(treeBefore, ['root_before'], { name: 'root_after' });
      expect(treeAfter).toEqual(expectedTreeAfter);
    });
